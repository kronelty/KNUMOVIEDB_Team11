...
<body>
	...
	<%
	sql = "SELECT * FROM MOVIE M WHERE M.type ='"+type+"'";
	sql += " AND M.title IN (SELECT TITLE FROM GENRE_OF G WHERE G.genre='"+genre+"')";
	sql += " AND M.title IN (SELECT TITLE FROM VERSION V WHERE V.country='"+country+"')";
	//평가한 대상은 검색결과에서 제외.
